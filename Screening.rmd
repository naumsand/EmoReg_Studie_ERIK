...
Die Datensets von dem Fragebogen Screening werden geladen.
...
Der Screeningfragebogen wurde f&uumlr `r screening[,.N]` Kinder ausgef&uumlllt, davon sind `r screening[FINISHED == 1,.N]` Frageb&oumlgen vollst&aumlndig. Nur diese Frageb&oumlgen werden in die Analyse einbezogen.
...
Der Altersdurchschnitt ist `r screening[,round(mean(age_in_years,na.rm = TRUE),1)]` (*SD* = `r  screening[,round(sd(age_in_years, na.rm = TRUE),2)]`). 
Die Altersspanne betr&oumlgt `r screening[,min(range(age_in_years, na.rm = TRUE))]` -- `r screening[,max(range(age_in_years, na.rm = TRUE))]` Jahre.
...
## HiPic Visualisierung
```{r HiPic_Visualisierung}
...
# traits - Visualisierung mit corr.plot 
